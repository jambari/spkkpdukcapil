...
function getNamak($id)
{
	include('config.php');
	$query  = "SELECT nama FROM kriteria WHERE id=$id";
	$result = mysqli_query($koneksi, $query);
...
//get nama alternatif for arsip Table
function getNamaa($id)
{
	include('config.php');
	$query  = "SELECT nama FROM alternatif WHERE id=$id";
	$result = mysqli_query($koneksi, $query);
...
function showTabelPerbandingan($jenis,$kriteria) {
	...
	<form class="w3-container w3-card" action="proses.php" method="post">
	<table class="w3-table ">
		<thead>
			<tr>
				<th colspan="3">Pilih yang lebih penting dan isi nilai Perbandingannya</th>
			</tr>
		</thead>
		<tbody>
	...
	?>
						<input class=""  type="text" name="bobot<?php echo $urut?>" value="<?php echo $nilai?>" required>
					</div>
				</td>
			</tr>
			<?php
	...
	<br><br><input class="w3-button w3-block w3-white w3-border w3-round w3-border-teal" type="submit" name="submit" value="submit">
	</p>
	</form>
